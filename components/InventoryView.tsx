...
const InventoryView: React.FC<InventoryViewProps> = ({ printers, onEdit, onDelete, isAdmin }) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-3 rounded-[28px] border border-gray-200 shadow-sm flex items-center gap-3">
        <div className="relative flex-1">
          <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
          <input
            type="text"
            placeholder="Buscar por Patrimônio, Série, Modelo ou Unidade..."
            className="w-full pl-14 pr-6 py-4 bg-gray-50/50 border-none rounded-2xl focus:ring-2 focus:ring-pcce font-bold text-sm placeholder:text-gray-400 transition-all"
            value={searchTerm}
        ...
        <div className="flex bg-gray-100/80 p-1 rounded-2xl">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all font-bold text-xs ${viewMode === 'list' ? 'bg-white shadow-sm text-pcce' : 'text-gray-500 hover:text-gray-800'}`}
          >
            <ListIcon size={16} />
            Tabela
          </button>
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-xl transition-all font-bold text-xs ${viewMode === 'grid' ? 'bg-white shadow-sm text-pcce' : 'text-gray-500 hover:text-gray-800'}`}
          >
            <LayoutGrid size={16} />
            Grade
          </button>
